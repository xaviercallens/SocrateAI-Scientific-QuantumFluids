...
set $\varepsilon$. There is a second stability theorem, due to the same authors with
Mileyko~\cite{CSEHM2010}, in which \emph{Wasserstein} distances between diagrams are bounded in terms of
an $L_p$ norm of $f-g$ (under a Lipschitz hypothesis on the functions and a bounded total-persistence
hypothesis on the space). An $L_p$ norm averages over the field; a handful of core pixels no longer
dominates it. And the left-hand side of that theorem is exactly the quantity this round learned to
certify: Table~\ref{tab:certificates}'s $W_1$, exhibited with a primal plan and dual potentials rather
than trusted from a solver.

\emph{Gate.} The exact statement -- the Lipschitz constant of a Gross--Pitaevskii density on a grid,
the total-persistence bound, the exponent on $\|f-g\|_p$ -- must be read from~\cite{CSEHM2010} and
written into a pre-registration before any number is computed; the constants are not recalled here on
purpose. \emph{Prediction to be pre-registered.} For the three resolution pairs, the $W_1$ bound is
