...
\paragraph{Part C ($L=128$): an admission failure, honestly reported.} Six runs at $e\in\{1.00,1.10,1.20\}$,
two seeds each, energy drift $\le1.1\times10^{-6}$. The isotropy admission built for $L\le64$
($R_L\le1.25$, $R_T\le1.1R_L$) rejects four of six: their transverse current fluctuations exceed the
longitudinal ones by a factor $2$--$4$, giving an unphysical negative $K=n_s\cdot2\pi/T$. Block-by-block
$T_b$, condensate fraction and vortex number are flat across the sampled window in every run, admitted or
not, so this is not a transient the window missed but a standing feature of the state reached -- most likely
because the warm-up window ($t_{\rm tr}=3000$), carried over unchanged from $L\le64$, does not let the
larger box's additional long-wavelength modes equilibrate. On the two admitted points ($e=1.10$:
$\eta K-1=0.284$; $e=1.20$: $\eta K-1=0.449$) the duality offset is \emph{larger} than at $L=64$
($0.12$, $0.20$) -- the opposite of the shrinking Hasenbusch trend the criterion was written to see -- but
with one surviving seed per energy and no admitted point at $e=1.00$, this is reported, not trusted: it is as
consistent with a biased subsample (admission keeps whichever seed happened to isotropise) as with a real
finite-size surprise. No crossing of $K=4$ is bracketed in the admitted range. \textbf{C1: fails as
pre-registered; C2: not bracketed; both void pending a longer equilibration window at this box size in a
future round} -- not executed here, since each run already costs $\approx14$~h.
