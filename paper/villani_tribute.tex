...
the paper describes. We also report a companion literature gate, run before any code, for a separate
question this library's kinetic-theory side had wanted to attach a Penrose-type stability criterion to
a Landau-liquid stability criterion of Pomeranchuk's: the two are equivalent for an
analogous three-dimensional system in the literature, but not, so far as we found, for the
two-dimensional case at hand, and the numerical parameters needed even to state the two-dimensional
version are not published anywhere we could reach. We stopped rather than derive either from scratch,
and report why.
\end{abstract}
...
inequality, Bakry--\'Emery theory, Otto calculus, the Boltzmann equation, Fisher information, a Poincar\'e
...
chain -- Boltzmann's H-theorem in the form Cover and Thomas give it, and the discrete skeleton of the
...
\bottomrule
\end{tabular}
\caption{The chain of 12 lemmas from the definitions to Theorem~1 at $K=0$; 16 declarations in total
with \lean{IsMidpoint}, \lean{midpointSet} and the entropy corollary of \S\ref{sec:kl}. Standard axiom
...
Only a triangle inequality, a set-difference cardinality identity, and Mathlib's canonical order
embedding of a finite subset are used beyond first principles: nothing about Theorem~1's proof needed
machinery this project did not already have on hand for other reasons.
...
\item \textbf{A discrete H-theorem for a finite Markov chain via majorization} (Schur-concavity of
entropy under a doubly stochastic map) was our first plan, abandoned mid-proof when \S\ref{sec:kl}'s
three-line route through existing Mathlib content turned out to give the sharper, more general fact
directly.
...
Kolomeitsev and Voskresensky~\cite{KolomeitsevVoskresensky2016}, derives exactly that equivalence by
