...
\begin{abstract}
\noindent
We present an open Lean~4/Mathlib library of 109 machine-checked theorems on the mathematical
...
\lean{BoseIntegral} & 5 & $\int_0^\infty t^{s-1}/(e^t-1)\,dt = \Gamma(s)\zeta(s)$ via Mathlib's
  Mellin machinery; $\int_0^\infty t^3/(e^t-1)\,dt = \pi^4/15$; the Debye $T^3$ coefficient \\
...
\bottomrule
\end{tabular}
\caption{Modules. Axiom footprint of all 109 theorems:
...
\item the remaining coefficients $C,D,E,K,L$ of~\cite{Godfrin2021} Eq.~(22), which need the same Bose
  integral at $s = 6\ldots10$ together with a Lagrange inversion --- the target with the most direct
  value to an experiment, since the paper reports a disagreement with two earlier published series.
\end{enumerate}
