...
\begin{abstract}
\noindent
We present an open Lean~4/Mathlib library of 136 machine-checked theorems on the mathematical
...
which it is actually computed, the discrete divergence-free condition that makes vortex-line
tracing well posed, sign and conservation structure of the Galerkin-truncated
...
the Navier--Stokes problem, imported as a real dependency, and an end-to-end check of a published
six-coefficient specific-heat series for superfluid $^4$He whose predecessors in the literature were
wrong---it is right. Every theorem carries a generated
...
failure case (\S\ref{sec:winding}), (iii) a reusable pattern for checking a printed series
expansion---truncation as nilpotency, with untrusted computer-algebra certificates checked by the
kernel (\S\ref{sec:series})---and (iv) the error taxonomy.
\end{keybox}
...
\lean{import QuantumFluids} brings in sixteen modules (Table~\ref{tab:modules}); the full
build is 8796 jobs, zero errors, zero \lean{sorry} in any default target.
...
\lean{PhononSeries} & 9 & the inverse series printed in~\cite{Godfrin2021} inverts the phonon dispersion
  modulo $e^8$; density of states modulo $e^9$; generated certificates (\S\ref{sec:series}) \\
\lean{PhononSpecificHeat} & 14 & $B_6,B_8,B_{10}$; $\zeta(6),\zeta(8),\zeta(10)$; the thermal Bose integral at
  every order; all six coefficients of~\cite{Godfrin2021} Eq.~(22) \\
...
\bottomrule
\end{tabular}
\caption{Modules. Axiom footprint of all 136 theorems:
...
\subsection{Vortex lines do not end}
\label{sec:noend}

Threshold-free line tracing---the method that replaced the confounded one of
\S\ref{sec:empirical}---walks a vortex line from grid cube to grid cube. That presupposes a line
entering a cube also leaves it, i.e.\ that the oriented windings of the six faces sum to zero: the
discrete form of $\nabla\!\cdot\!\omega = 0$. Our own implementation assumed it without checking.

The proof is combinatorial, not homological. Model a closed surface by its oriented boundary edges,
as a family in which every edge occurs with both orientations (a fixed-point-free involution $\sigma$
with $u_{\sigma i} = v_i$, $v_{\sigma i} = u_i$); the raw phase differences then cancel in pairs.

\begin{theorem}[\lean{balanced\_sum\_eq\_zero}]
For a balanced family of oriented edges on which no edge sits at the branch cut,
$\sum_i \mathrm{pd}(u_i, v_i) = 0$.
\end{theorem}
The hypothesis is again exactly ``no phase difference equals $\pi$''
(\lean{cut\_free\_of\_ne\_pi}), and removing it makes the file fail to compile---as it must, since
there the two traversals contribute $2\pi$ rather than cancelling. The same defect that breaks
antisymmetry in \S\ref{sec:winding} is what would let a traced line terminate in mid-air.
...
\subsection{A six-coefficient series, checked end to end}
\label{sec:series}

That paper's Eq.~(22) gives $C_V = AT^3+CT^5+DT^6+ET^7+KT^8+LT^9$ in closed form for
$\varepsilon = ck(1+\alpha_2k^2+\dots+\alpha_6k^6)$, via an inverse series $k(\omega)$ printed through
$(\omega/c)^7$. We had earlier reproduced the six printed \emph{numbers} from the six printed
\emph{formulas}, which tests arithmetic and nothing else; the formulas are where an error would live.
We therefore derived them independently (series reversion, density of states, Bose integrals) and then
formalized the derivation. \textbf{All six coefficients and all seven printed inverse-series
coefficients are correct.} We report this as what it is, a confirmation.

The formal statement of ``equal up to $O(u^8)$'' avoids analysis entirely: it is an identity in an
arbitrary commutative ring with a nilpotent element, which specialises to $A[[u]]/(u^8)$.
\begin{verbatim}
theorem dispersion_kInv {R : Type*} [CommRing R]
    (a1 a2 a3 a4 a5 a6 e : R) (h : e ^ 8 = 0) :
    kInv a1 a2 a3 a4 a5 a6 e *
      (1 + a1 * kInv a1 a2 a3 a4 a5 a6 e + ... + a6 * kInv a1 a2 a3 a4 a5 a6 e ^ 6) = e
\end{verbatim}
Proved directly, this needs a remainder of $4918$ monomials. Instead the powers $k^2,\dots,k^7$ are
truncated one at a time, each step closed by \lean{linear\_combination r * h} with a certificate $r$ of
$40$--$202$ terms computed by a computer-algebra script. The script is untrusted: a wrong certificate
is rejected by the kernel, and we checked that it is (a single altered integer in the printed $\eta$,
and in each of $D$, $K$, $L$, fails to compile). The density of states then exhibits the brackets of the
printed coefficients---$7(4\alpha_2^2-\alpha_4)$, $8(9\alpha_2\alpha_3-\alpha_5)$,
$-3(55\alpha_2^3-30\alpha_2\alpha_4-15\alpha_3^2+3\alpha_6)$---and its vanishing $e^3$ term is the
absent $BT^4$.

The integrals come from \lean{bose\_integral\_nat}, $\int_0^\infty t^n/(e^t-1)\,dt = n!\,\zeta(n+1)$,
rescaled to temperature by Mathlib's Mellin scaling lemma. Even orders need $\zeta(6),\zeta(8),\zeta(10)$,
hence $B_6,B_8,B_{10}$; Mathlib's table of Bernoulli numbers stops at $B_4$, and unfolding the
defining recursion under \lean{simp} loops, so they are proved one at a time, each from its
predecessors. The odd orders give $\zeta(7)$ and $\zeta(9)$, which have no known closed form: this is
\emph{why} the printed $D$ and $K$ carry an explicit $\zeta$ while $A,C,E,L$ carry powers of $\pi$, and
it means a disagreement between published series in those two coefficients cannot be adjudicated by
exact algebra, only numerically.

\paragraph{What this does not establish.} Three things, stated in the module's header. The density-of-states
brackets are passed to the final theorem by inspection, not by a coefficient-extraction lemma.
Term-by-term integration is used, each term's integral being proved. And that the series is an
asymptotic expansion of $C_V$ for the \emph{measured} dispersion---upper limit at infinity, roton branch
dropped---is a statement about a measured function that no proof assistant can certify. This check was
also not pre-registered, contrary to the project's own rule; a symbolic match admits no tuning, so we do
not think the result is weakened, but we record the lapse.

\paragraph{Apparent misprints, and a caution about them.} We also noted two apparent misprints (and one
unit inconsistency) elsewhere in the preprint, each demonstrated against the paper's own tables. The
arXiv record has a single version, dated before the journal's acceptance, and the version of record is
not accessible to us; the journal page lists no erratum. These may therefore already be corrected in
print, and we have communicated nothing to the authors until that is checked.
...
\item from modes to fields: that a finite Galerkin state defines a continuous periodic field with
  those Fourier coefficients. The $n$-torus Fourier inversion theorem needed is not in Mathlib in this
  form; one exists, \lean{sorry}-free, inside Anthropic's Lean development of Fermat's Last
  Theorem~\cite{AnthropicFLT}, one toolchain release behind ours, so it is a port rather than an import;
\item the two gaps of \S\ref{sec:series}: coefficient extraction in \lean{PowerSeries}, and the
  asymptotic statement for a dispersion agreeing with the series to $O(k^8)$.
\end{enumerate}

\noindent A fuller roadmap, with a literature gate and a kill criterion per target and an assessment of
what three existing Lean developments can and cannot contribute, is in the repository
(\texttt{docs/LEAN4\_QUANTUM\_FLUIDS\_ROADMAP.md}).

\section{Related work}
\label{sec:related}

\paragraph{Formalized physics.} Lean~4~\cite{Lean4} and Mathlib~\cite{Mathlib} now host physics
libraries of some size: HepLean, now PhysLean, for high-energy physics~\cite{HepLean}, and a
formalization of chemical-physics derivations~\cite{Bobbin2024} whose motivation---derivations in the
applied literature are stated, not proved---is the one we arrived at in \S\ref{sec:godfrin}. We know of
no prior formal development for superfluids or condensates. Our library is small beside these, and its
claim is narrower: statements chosen because a simulation code or an experimental analysis depends on
them.

\paragraph{Verified numerics.} The closest methodological relatives are mechanized correctness proofs of
numerical programs: the wave-equation scheme of Boldo et al.~\cite{Boldo2013} and Immler's verified ODE
solver~\cite{Immler2018}. Those verify an implementation down to floating point. Ours verifies only the
\emph{mathematical} step of an algorithm (\S\ref{sec:winding}): that the winding sum is an integer and
when neighbouring plaquettes cancel. No code is extracted and no rounding is modelled.

\paragraph{Certificates from untrusted tools.} Using computer algebra as an oracle whose output the
kernel checks is standard practice---it is how \lean{polyrith} and \lean{linear\_combination} are meant
to be used. \S\ref{sec:series} applies it to truncated series by moving the truncation into the ring.

\paragraph{Large formalizations used here.} We import one module of~\cite{OpenAINSE} and evaluated
\cite{AnthropicFLT} as a source of general analysis. Both use the two-kernel Comparator
check~\cite{Comparator} that we adopted. What distinguishes an experience report from those is scale
in the other direction: at $136$ theorems every statement can be, and was, read by a person, and most of
what we report went wrong at the level of \emph{which statement}, not which proof.
