...
That coefficient is no longer an isolated case. \lean{bose\_integral\_nat} gives
$\int_0^\infty t^n/(e^t-1)\,dt = n!\,\zeta(n+1)$ at every natural order, so all six coefficients of
Eq.~(22) rest on one proved identity, and \lean{bose\_integral\_even} makes the even orders explicit
through Mathlib's Bernoulli formula for $\zeta(2k)$. Doing so surfaced a structural fact about that
series worth stating plainly: $D$ and $K$ carry $\zeta(7)$ and $\zeta(9)$, which have no known
closed form. They are not missing from the library; they are missing from mathematics. So $A, C, E,
L$ can be given exactly while $D$ and $K$ admit only numerical evaluation --- which means a
disagreement between published series in those two coefficients cannot be adjudicated by exact
algebra at all. We had not seen that noted, and it bears directly on the comparison the paper raises.
