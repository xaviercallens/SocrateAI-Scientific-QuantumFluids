...
\begin{abstract}
\noindent
We present an open Lean~4/Mathlib library of 89 machine-checked theorems on the mathematical
...
useless in the limit that mattered, and eleven theorems that had silently escaped audit. It
...
\lean{import QuantumFluids} brings in twelve modules (Table~\ref{tab:modules}); the full
build is 8788 jobs, zero errors, zero \lean{sorry} in any default target.
...
module & thms & content \\
\midrule
\lean{VortexWinding} & 7 & loop quantization of the principal-branch phase sum; edge
  cancellation and its exact failure case \\
...
\bottomrule
\end{tabular}
\caption{Modules. Axiom footprint of all 89 theorems:
...
\item A survey found no public dilute-condensate dataset resolving $\xi$ by five cells or
  ...
  bound is detectably wrong). \CONVERGENCE
\end{enumerate}
We think the hypothesis was physically naive: annihilation and reconnection are processes
that pass continuously through separations below $\xi$, so quantization constrains
circulation, not distance.
...
\S\ref{sec:caught}, items~5--6. The $^4$He dispersion data are those of Godfrin et
