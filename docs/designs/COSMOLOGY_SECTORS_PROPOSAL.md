...
* Rindler-Daller & Shapiro 2012 (DOI 10.1111/j.1365-2966.2012.20588.x): **a sector vortex additionally needs
  strong self-interaction** — their result is "vortices cannot form for vanishing self-interaction (i.e. when
  λ_dB ≲ R)". In the pure-quantum-pressure regime (fuzzy DM, Hui et al.'s setting) interference vortices form
  regardless of rotation; a genuine rotation-sourced sector vortex additionally requires the Thomas–Fermi
  regime. Schobesberger, Rindler-Daller & Shapiro 2021 (DOI 10.1093/mnras/stab1153) give the halo core version
  of this quantitatively: "for typical halo spin parameters, angular momentum per particle is below ℏ, the
  minimum required even for one singly-quantized vortex in the centre" — vortices, when they appear at all in
  cosmological FDM simulations, form in the turbulent envelope, not the soliton core.
* Brax & Valageas 2025 (arXiv:2502.12100), independent 3D rotating-halo simulations, find both populations we
  distinguish: "a regular lattice of vertical vortex lines, aligned with the total angular momentum" in the
  relaxed core (the sector) and "a tangle of intertwining and curved vortex lines of any direction" with random
  ±1 windings in the envelope (the pairs), and state our exact open problem: "a difficulty for numerical
  computations would be to distinguish the extended vortex lines that link distant halos... from the chaotic
  maze of vortices generated by the random interferences." Zhou et al. pose the same gap on the lensing side:
  "it will be important for a future study to identify ways to distinguish if the magnification anomalies are
  caused by underdensity 'holes' (due to vortices) or overdensity subhalos" — and every one of their simulated
  haloes has Ω > 0 built into the initial condition; none tests Ω = 0 with a compensated vortex population at
  matched energy. **No published simulation has yet built the discriminating instrument this proposal designs.**
  The vortex density π/λ_dB² itself is not special to dark matter: it is Berry & Dennis's 25-year-old result for
  any isotropic random wave field (DOI 10.1098/rspa.2000.0602), with perimeter-law charge screening proved by
  Dennis 2003 (DOI 10.1088/0305-4470/36/24/301) — power-law, not exponential/Debye (the naive "screening length"
  diverges; Houston, Gradhand & Dennis, arXiv:1612.01839).
...
3. *Conservation and the barrier.* A non-zero halo winding can change only by a vortex line crossing the
   ...
   imposed W and a matched non-rotating arm at equal energy (the design of round 2, transplanted). One real
   exception, found in the lab (Eckel et al. 2014, DOI 10.1038/nature12958, on a **bounded** ring): a pair with
   one member crossing the boundary and escaping is the literal microscopic mechanism by which net winding
   changes by one unit — "pairs are invisible to the sector" needs this caveat off a periodic torus.
