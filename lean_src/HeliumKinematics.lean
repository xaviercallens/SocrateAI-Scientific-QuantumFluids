...
theorem phase_velocity_excess (c α₂ α₃ α₄ k : ℝ) (hk : k ≠ 0) :
  ...
  unfold seriesDisp; field_simp; ring
...
theorem group_velocity_excess (c α₂ α₃ α₄ k : ℝ) :
  ...
  have h : HasDerivAt (fun x : ℝ => c * x + c * α₂ * x ^ 3 + c * α₃ * x ^ 4 + c * α₄ * x ^ 5)
      (c * 1 + c * α₂ * (3 * k ^ (3 - 1) * 1) + c * α₃ * (4 * k ^ (4 - 1) * 1)
        + c * α₄ * (5 * k ^ (5 - 1) * 1)) k := by
    have hid := hasDerivAt_id k
    exact (((hid.const_mul c).add ((hid.pow 3).const_mul (c * α₂))).add
      ((hid.pow 4).const_mul (c * α₃))).add ((hid.pow 5).const_mul (c * α₄))
  have hf : seriesDisp c α₂ α₃ α₄
      = fun x : ℝ => c * x + c * α₂ * x ^ 3 + c * α₃ * x ^ 4 + c * α₄ * x ^ 5 := by
    funext x; unfold seriesDisp; ring
  rw [hf]
...
#print axioms QuantumFluids.HeliumKinematics.symmetric_split_excess
#print axioms QuantumFluids.HeliumKinematics.phase_velocity_excess
#print axioms QuantumFluids.HeliumKinematics.group_velocity_excess
