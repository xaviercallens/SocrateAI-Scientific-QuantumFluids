...
| CLAIM-052 | — → VERIFIED (mathematics; explicitly not the anomaly argument) | 2026-09-26 | `lean_src/SectorDuality.lean`, 3 theorems, standard axioms, Comparator accepted (nanoda + Lean), 3 negative controls fail. Cosmology brief Tier 1 item 1: the finite-cyclic-group counterpart of `CompactBoson`'s duality-as-reindexing statement, built on Mathlib's own `ZMod.dft`/`ZMod.dft_dft` (David Loeffler) rather than re-deriving Fourier inversion. `sectorDuality_bijective`: the duality is a bijection of Z_N-sector-weighted data (a `LinearEquiv`, the finite-group analogue of `CompactBoson.swap`). `sectorDuality_sq_ne_id`: for N >= 2 the duality squared is NOT the identity -- Aasen-Mong-Fendley's 'not a symmetry in the traditional sense' and Shao's 'away from the critical point KW duality... is a map from the high-T phase to the low-T phase', read off `dft_dft`'s N.(reflection) shape. `kramersWannier_gauging_sq`: the N=2 case spelled out -- gauging a Z2 sector symmetry twice returns TWICE the original weighting, matching Choi-Cordova-Hsin-Lam-Shao's and Gaiotto-Kapustin-Seiberg-Willett's 'gauging a non-anomalous Z2 twice gives back the original theory' (up to the factor |G|=2 the finite sum produces). Explicitly NOT claimed: no anomaly-inflow/SPT-obstruction argument (needs cohomology this file does not touch); the criterion for which fixed points are forced transitions remains prose in `duality_sector.tex`'s addendum. Bug found and fixed in `scripts/make_comparator_challenges.py`: the challenge generator cut theorem statements at the FIRST `:=` found anywhere, mistaking a named argument's `:=` (e.g. `ZMod.dft (N := N)`) inside the signature for the proof-starting `:=`; fixed to track bracket depth and cut only at a depth-0 `:=`. All 30 modules regenerated; only SectorDuality's challenge was actually broken by the bug (confirmed by diff: every other module's challenge file is byte-identical except ChargeLattice's docstring, unrelated, already-stale from an earlier edit). Library 253 theorems / 30 modules. |
| CLAIM-051 | — → CONFIRMED (on a recalibrated statistic) | 2026-09-26 | Round 4's B2 (pairs visible in density, >= 30% of baseline) FAILED as pre-registered at e=0.90 (21-25%) because the criterion compared against each base's own baseline S_band(0), which varies an order of magnitude with the base's pre-existing thermal vortex population (N_v(0) = 0, 6.8, 4.6 at e=0.60, 0.90 s11, 0.90 s12). The density power ADDED PER INJECTED VORTEX, dS_band/dN_v, is the properly calibrated statistic: it agrees to 8% across the two e=0.90 seeds (2.95e-4 vs 2.73e-4), and is ~1.9x larger at e=0.60 (5.31e-4), physically sensible (a vortex core is a deeper relative dip against a more phase-coherent, higher-condensate-fraction background at lower energy). B2's qualitative claim ('pairs are visible in density') is confirmed on the properly calibrated statistic; the original threshold failure was a calibration artefact of the pre-registration, not a physics failure. Cosmology brief Tier 1 item 3, closed on existing data, no new runs. |
| CLAIM-050 | — → PUBLISHED | 2026-09-26 | v1.13.0 published on Zenodo, DOI 10.5281/zenodo.22976724 (new version of 22967670, concept 10.5281/zenodo.22855581): archive + 5 PDFs incl. `paper/cosmology_sectors.pdf` (CLAIM-047/048/049) and `paper/duality_sector.pdf` with its dated addendum. GitHub release v1.13.0. Hugging Face dataset updated. Library 250 theorems / 29 modules. |
| CLAIM-049 | — → CONFIRMED (3 of 4 pass; B2 fails a threshold, not a direction) | 2026-09-26 | Round 4 Part B (torus, 12 runs, drift <= 2.2e-7): B1 the imposed sector (winding 3, +177.8 energy) changes the core-scale density power by 0.3-3.6% (PASS, 3/3, threshold 15%); B4 the same 16 injected pairs carry the identical density signature with or without the sector superposed, 1-4% (PASS, 3/3, the crux comparison); B3 the sector is read only by the phase (torus winding exact, 100% of samples, no slip in 1500 time units) and the momentum fraction 0.66-0.92 (PASS, threshold 0.15); B2 the pairs alone raise the density power 21-118%, an order of magnitude more than the sector -- PASS at e=0.60 (118%) but FAILS the pre-set 30% threshold at e=0.90 (21-25%, base-dependent thermal pair dilution, direction/magnitude not in doubt). Composite: a density-only observable cannot distinguish a sector-carrying configuration from a non-rotating one at matched core content, on this torus -- the transplant of Zhou et al. 2025's and Brax & Valageas 2025's own open question, answered here. Part A (random-phase halo model, complete: density pi/lambda^2 to 1%, only +-1 windings, Var W(R) ~ R^1.00 perimeter law). `PGPE_R4_RESULTS.md`; `cosmology_sectors.tex` prediction box updated with the result. |
| CLAIM-048 | — → FAIL/INCONCLUSIVE (honest negative) | 2026-09-26 | Round-3 Part C (L=128 finite-size ladder): 4 of 6 runs fail the isotropy admission built for L≤64 (R_T/R_L 2-4x, unphysical negative K); block-by-block T_b/cond/n_v flat, so this is a standing feature not a transient the window missed -- most likely t_tr=3000 too short for the larger box's extra long-wavelength modes to equilibrate, not anticipated in the prereg. C1 (duality-offset ratio, on the 2 admitted points only): 2.29, opposite direction to the pre-registered [0.6,1.0] shrinking prediction -- reported, not trusted, given the tiny biased-by-admission sample. C2 (T_BKT(128) <= T_BKT(64)): not bracketed. Both void pending a longer-equilibration rerun (not executed: each L=128 run costs ~14h). `PGPE_R3_RESULTS.md`, `sector_temperature.tex` Part C section filled honestly. |
| CLAIM-047 | — → VERIFIED (mathematics; the cosmological reading is stated as a reading) | 2026-09-25 | `lean_src/ChargeLattice.lean`, 15 theorems, standard axioms, Comparator accepted (nanoda + Lean), 3 negative controls fail. Part 1 (Moore 1998, K3 × T² dyons): for any symmetric ℤ-bilinear form, D(p,q) = B(p,q)² − B(p,p)B(q,q) satisfies D(ap+bq, cp+dq) = (ad−bc)² D(p,q) — SL(2,ℤ)-invariant; S-swap, scaling t⁴, D(p,kp) = 0, horizon area √(−D) orbit-invariant. Part 2 (Bousso–Polchinski 2000; Kaloper 2025): Λ(n) = Λ_bare + ½Σ n_i²q_i²; one nucleation changes Λ by −(n_i − ½)q_i², lowers it iff n_i ≥ 1; floor at n = 0; Kaloper V(N) = ½X((1−N)θ̂)² vanishes iff N = 1 and the discharge from N ≤ 1 terminates after exactly 1−N steps. Reading (`docs/designs/COSMOLOGY_SECTORS_PROPOSAL.md`): the observable is a function of the integer sector, the duality reindexes the lattice and preserves it (K3: the attractor geometry is an OUTPUT of the charges — the fluid ↔ K3 transfer excluded in v1.9–v1.11 was an attempt to transfer an input), the dynamics is a slip cascade on the lattice (Kaloper = our e0.60 winding-2 decay written for the vacuum); for wave dark matter the only sector-level halo observable is the net circulation (Zhou 2025 lattice) — interference rings (Hui 2020, winding ±1, one per λ_dB³) are dipoles invisible at R ≫ λ_dB by `ScaleResolvedWinding`. No cosmological number claimed; topological-defect DE recorded as excluded (Ω_s < 0.009, Cheng–Di Valentino–Visinelli 2025). Library 250 theorems / 29 modules. |
| CLAIM-046 | — → PUBLISHED | 2026-09-25 | v1.12.0 published on Zenodo, DOI 10.5281/zenodo.22967670 (new version of 22951842, concept 10.5281/zenodo.22855581): archive + 4 PDFs incl. `paper/duality_sector.pdf` (CLAIM-045) and `paper/sector_temperature.pdf` with round 3 Parts A/D. GitHub release v1.12.0. Hugging Face dataset `callensxavier/socrateai-quantumfluids-causal-topology` updated to v1.12.0. Library 235 theorems / 28 modules. Part C (L = 128) pending, v1.13. |
| CLAIM-045 | — → VERIFIED (mathematics; the generalisation is a reading, stated as such) | 2026-09-25 | `lean_src/CompactBoson.lean`, 12 theorems, standard axioms, Comparator accepted (nanoda + Lean), 3 negative controls fail: T-duality of the compact boson on its sectors — dim(n,w;R) = dim(w,n;2/R); the sector sum Σ_{ℤ²} F(Δ,s) invariant under R ↦ 2/R by reindexing (`Equiv.tsum_eq`, no convergence hypothesis); Δ_e·Δ_m = 1/4 for every R (= η·n_sλ² = 1, the relation rounds 2–3 measured to 12–27 %); self-dual radius √2 (Δ = 1/2 each) vs vortex-marginality radius 2√2 (Δ_m = 2, η = 1/4): BKT is not self-dual. Reading (notes §14, sector paper §duality): a duality is a Fourier/reindexing map on the sector group (Savit 1980) — a structure, verified, not a cause; the cause is the sector; a self-dual point pins a transition only when the dual sectors are simultaneously critical with discrete fixed points (Ising), not on a fixed line (BKT); string-gas moduli (Brandenberger–Vafa) = preferred equilibrium with both sectors populated, Hagedorn exit = winding annihilation; SIT self-duality = constraint on the fixed point (Fisher 1990). No novelty claimed. Library 235 theorems / 28 modules. |
| CLAIM-044 | — → PUBLISHED | 2026-09-25 | v1.11.0 published on Zenodo, DOI 10.5281/zenodo.22951842 (new version of 22949272, concept 10.5281/zenodo.22855581): archive + 8 PDFs incl. `paper/sector_temperature.pdf` (CLAIM-042/043). GitHub release v1.11.0. Library 223 theorems / 26 modules. |
| CLAIM-043 | — → PROPOSAL + two external tests run (one PASS, three FAIL-as-written) | 2026-09-25 | `paper/sector_temperature.tex` (hypothesis F: topological sectors carry their own temperature; Onsager two-temperature picture, established: Gauthier/Johnstone Science 2019, Groszek PRL 2018). Instrument: torus point-vortex energy (Weiss–McWilliams 1991 eq. 12) with known answers (dipole slope 2 ln 2 exact; W&M N=6 rms 2.985 vs 2.90; mean = additive convention); canonical Metropolis calibration DONE: closure PASS at N=4–16 (read 1.14–1.31 for set 1.2; a 1500-sweep run failed at N=6,8 = precision); fluctuation identity OK at β ≥ 0.3, FAILS at β ≤ 0 (negative-T branch not equilibrated; thermometer valid for β ≥ 0.3 only). Post-hoc calibrated reading t=1500: T_v/T_bath = 5.5, 8.4 (e=0.90 V); e=0.60 V at |β_v| ≲ 0.1 (T_v ≫ T_b, sign not read); thermal arms below calibrated range. Energy budget (field, NAB): bath loss → low-k incompressible (+63 vs −50 at e=0.60), not sound; point-vortex cross-N budget void. Post-hoc placement of round-2 finals: imprinted e=0.60 configuration at −0.27σ of the random-configuration energy (β_v ≈ 0) beside a β_bath ≈ 10 bath; thermal arms −4.5…−6σ; numerical T_v NOT quoted. External data (predictions written before reading): **P5** Gauthier et al. 2019 vortex positions (Zenodo 2548958, CC BY): l/l0 decay time τ = 6.36, 5.41, 3.59, 3.46, 3.46, 2.85 s for condensate fractions 75→18 % — monotone yes, ratio 2.2 vs threshold 3: **FAIL as written**. **P6** Christodoulou et al. 2021 (Apollo 10.17863/CAM.66056, CC BY): part 1 PASS (D_s = 2.99, 4.93 at D/D_c ≈ 1 vs 4); part 2 FAIL as written (5 of 6 superfluid points overlay our PGPE ladder with D/D_c = T_BKT/T within 0.2–14 %, no free parameter; the point on the jump 33 %, 1.9σ). NO NOVEL PHYSICS. Data placed: Christodoulou source files committed under data/external; Gauthier 5 MB zip left where it downloaded (lean_src/data/external, not committed; owner declined a move). |
| CLAIM-042 | — → VERIFIED (combinatorics) | 2026-09-25 | `lean_src/SectorTemperature.lean`, 8 theorems, standard axioms, Comparator accepted (nanoda + Lean), 3 negative controls fail: global count = Σ sector counts; log n_W ≤ log N; **mediant inequality** — the global inverse-temperature factor N(E+1)/N(E) lies between min and max of the sector factors n_W(E+1)/n_W(E) (a sector-blind thermometer reads a weighted mean; equal E ⇏ equal T once a conserved label exists); single-sector equality; sector invariance under no-slip evolution (via `loop_sum_const_of_no_slip`). This is the exact reason round 2's I2 clause ('and measured T') was wrong in principle. NOT proved: which temperature the equipartition estimator reads on a continuum field. Library 223 theorems / 26 modules. |
| CLAIM-041 | — → PUBLISHED | 2026-09-25 | v1.10.0 published on Zenodo, DOI 10.5281/zenodo.22949272 (new version of 22904619, concept 10.5281/zenodo.22855581): archive + 7 PDFs incl. `paper/causal_topology.pdf` (CLAIM-039/040) and `paper/astro_topological_measurement.pdf` (methods; no astrophysical number claimed). GitHub release v1.10.0. Library 215 theorems / 25 modules, all Comparator-accepted. Note: the deposited PDFs cite the causal paper as 'in preparation'; the bib source now carries the DOI. |
| CLAIM-040 | — → VERIFIED IN PART (pre-registered; composite causal claim NOT made as worded; post-hoc reading stated as such) | 2026-09-25 | Round 2, `docs/designs/PGPE_R2_RESULTS.md` (prereg f8e9773, amendments R2-A1..A4). PART I, energy-matched intervention (4 imprinted vortex pairs vs the same ΔE as phonons vs untouched, 3 bases, controls C1/C1b/C2/C3/C4 pass): I1 PASS ×3 (condensate lower in V by 0.49–0.73 vs ≥ 0.15), I4 PASS ×3 (η ratio 8.6–43.7 vs ≥ 1.5; V exponential, P algebraic), I3 PASS ×2 (Q gap 0.55–0.66; not evaluable at e=0.60), **I2 FAIL ×3**: T_V < T_P by 5.5/9.2/16 %, more than the surplus (1.5–3.3 %) and not among R2-A3's pre-data readings ⇒ the composite claim 'at fixed E,N,P and measured T' is NOT made. Post hoc (labelled): core-spectrum contamination REFUTED (7e-5 vs 3e-2 occupation), Doppler INCONCLUSIVE, V colder than the untouched arm by 4 % while P warmer by 1.5–3.5 % ⇒ the topological configuration draws heat from the bath (E conserved to 2e-7; ⟨N_v⟩ 14–16 vs 8+5); that sign cannot explain the effect. Effect per unit energy: 15–60× (condensate), 9–11× (η). Eight imprinted vortices persist t=1500 with ZERO annihilations at e=0.60; condensate recovers while N_v constant and Q falls (dipoles contract). PART II, heating ladder from ordered states (18 runs, R2-A4 widened e to 1.40 after the log showed the crossing above 1.20): L1 PASS T_BKT(L=64)=0.821 (between e=1.25/1.30), L2 PASS η=0.307 at the crossing, L3 PASS η·n_sλ²=1.12–1.22 on six energies (+12–22 % systematic); C5 FAIL: ladder e=1.20 has n_sλ²=5.24 vs quench 3.04 at equal N_v≈90 ⇒ round 1's T_BKT≈0.72 (CLAIM-038) WITHDRAWN as quench-biased. PART III, L=32 (15 runs): F1 PASS T_BKT(32)=0.893 ≥ 0.821, F2 PASS slope 32 vs 13; duality 1.14–1.27. nλ² at T_BKT = 7.65 vs ln380 = 5.94 (+29 %; cutoff dependence still unquantified). Part IV re-analysis: D3 stays FAIL. NO NOVEL PHYSICS. Papers: `paper/causal_topology.pdf`, `paper/astro_topological_measurement.pdf` (methods; no astrophysical number). |
| CLAIM-039 | — → VERIFIED (mathematics; the physical link is CLAIM-040, pending) | 2026-09-25 | Causal-topology stream, `docs/designs/CAUSAL_TOPOLOGY.md`: three Lean modules, 28 theorems, standard axioms, Comparator accepted by nanoda and the Lean kernel, 12 negative controls fail as required. `TopologicalProtection` (13): winding stable under perturbations that push no edge through π; continuous-time conservation by IVT on the integer-valued loop sum; winding change ⇒ phase slip (Anderson); discrete topological Kelvin theorem; XY-ring mountain pass E ≥ −(n−2)J for any winding-changing evolution; barrier from the twisted w=1 state ≥ 2J − 2π²J/n > 0 for n ≥ 10 — FALSE at n = 9 (9cos40° = 6.894 < 7), which is a negative control. `ScaleResolvedWinding` (6): discrete Stokes (boundary sum = Σ plaquette windings when no interior edge sits at π), boundary = 2π × net charge, dipole invisible from any enclosing scale, single core visible at every scale — W(R) = net charge inside R (thought experiment B). `ContinuumWinding` (9, 'Rome'): for a continuous closed loop of angles, path lifting through ℝ → ℝ/2πℤ (`AddCircle.isCoveringMap_coe`, `IsCoveringMap.liftPath`) gives an integer degree and Heine–Cantor a threshold n₀ beyond which the sampled principal-branch loop sum is exactly 2πw for ANY representatives; `detector_correct`: arg of a nowhere-vanishing closed loop of field values ⇒ the phase-winding detector returns 2πw for all n ≥ n₀ — closes the gap `VortexWinding.lean` declared NOT PROVED since v1.2. NOT PROVED (stated in the files): the argument principle (degree counts zeros inside; not in pinned Mathlib for continuous maps), a quantitative grid-to-n₀ bound, minimality of the twisted state in its sector. NO NOVEL PHYSICS (Anderson 1966, Langer–Fisher 1967, KT 1973). Library 215 theorems / 25 modules. |
