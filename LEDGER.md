...
| CLAIM-022 | PENDING → VERIFIED | 2026-09-19 | TIER A (structure) + TIER B (numeric mirror): `GPGalerkin.lean`, 5 theorems, footprint [propext, Classical.choice, Quot.sound]. For EVERY finite truncation Λ of an additive group (Z^3 included): the projected GP interaction sum Q = Σ conj(ψ_k) N_k equals Σ_q |A_q|² (A_q = FT of |ψ|² on Λ), hence is real and ≥ 0; Σ_k Im(conj ψ_k G_k) = 0 (algebraic core of mass conservation); kinetic part ≤ E when E = kinetic + (g/2)Q, g ≥ 0. NOT PROVED: conservation of E along the flow (Hamiltonian gradient identity), any existence/dynamics statement, any Navier–Stokes comparison (prose only). Numeric mirror on random data: Q = Σ|A|² = 4762.78, mass rate 0; negative control (nonlinearity without the conjugate) gives complex Q, Q ≠ Σ|A|², rate 843. |
